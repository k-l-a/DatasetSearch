...
default_l2 = 0.008476011455059052
default_rate = 0.09977829739451409
default_iter = 356.55484375
default_max_leaf = 202.45919181823731
default_samples_leaf = 28.330279693603515
...
class GradientBoostingClassifier(AutoSklearnClassificationAlgorithm):
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        learning_rate = UniformFloatHyperparameter(
            name="learning_rate", lower=0.01, upper=1, default_value=default_rate, log=True)
        max_iter = UniformIntegerHyperparameter(
            "max_iter", 32, 512, default_value=int(default_iter))
        min_samples_leaf = UniformIntegerHyperparameter(
            name="min_samples_leaf", lower=1, upper=200, default_value=int(default_samples_leaf), log=True)
        max_depth = UnParametrizedHyperparameter(
            name="max_depth", value="None")
        max_leaf_nodes = UniformIntegerHyperparameter(
            name="max_leaf_nodes", lower=3, upper=2047, default_value=int(default_max_leaf), log=True)
        max_bins = Constant("max_bins", 256)
        l2_regularization = UniformFloatHyperparameter(
            name="l2_regularization", lower=1E-10, upper=1, default_value=default_l2, log=True)
